...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='mask.py')
    ...
    opt.save_json = opt.save_json or any([x in opt.data for x in ['fruit.data', 'coco2014.data', 'coco2017.data']])
    print(opt)
    ...
    if opt.task == 'test':  # task = 'test', 'study', 'benchmark'
        ...
        for i in [320, 416, 512, 608]:
            for j in [0.5, 0.7]:
                t = time.time()
                r = mask(opt.cfg, opt.data, opt.weights, opt.batch_size, i, opt.conf_thres, j, opt.save_json)[0]
        ...
        for i in x:
            t = time.time()
            r = mask(opt.cfg, opt.data, opt.weights, opt.batch_size, opt.img_size, opt.conf_thres, i, opt.save_json)[0]
            y.append(r + (time.time() - t,))
